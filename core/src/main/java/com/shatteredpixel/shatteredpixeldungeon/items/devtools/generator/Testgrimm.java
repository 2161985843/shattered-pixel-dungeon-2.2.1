...
public class Testgrimm extends TestGenerator {
    {
    ...
    public void execute(Hero hero, String action) {
        ...
        if (action.equals(AC_GIVE)) {
            GameScene.show(new Testgrimm.SettingsWindow());
        }
    }

    private Weapon modifyWeapon(Weapon w) {
        if (w instanceof MagesStaff)
            w = new MagesStaff((Wand) Generator.random(Generator.Category.WAND));
        w.level(levelToGen);
    ...
    private void createWeapon() {
        Weapon grimm = Reflection.newInstance(weaponList(tier)[weapon_id]);
        grimm = modifyWeapon(grimm);
        grimm.identify();
        if (grimm.collect()) {
            GLog.i(Messages.get(this, "collect_success", grimm.name()));
        } else {
            grimm.doDrop(curUser);
        }
    }
    ...
    private Class<? extends Weapon>[] weaponList(int t) {
        switch (t) {
            case 1:
            default:
                return (Class<? extends Weapon>[]) Generator.Category.GRM_T1.classes.clone();
            case 2:
    ...
    private class SettingsWindow extends Window {
        ...
        private String enchantDesc() {
            ...
            Class<? extends Weapon.Enchantment> ench = generateEnchant(enchant_rarity, enchant_id);
            desc += Messages.get(Testgrimm.class, key, (ench == null ? Messages.get(Testgrimm.class, "null_enchant") : currentEnchName(ench)));
            return desc;
        }
    }
}